...
check_env() {
  ...
  read -p "确定要卸载「三省六部」系统并清理相关 Agent 数据吗？(y/N) " -n 1 -r
  echo
  if [[ ! $REPLY =~ ^[Yy]$ ]]; then
    info "已取消卸载"
    exit 0
...
stop_services() {
  ...
  # 停止 run_loop.sh
  if pgrep -f "scripts/run_loop.sh" > /dev/null; then
    pkill -f "scripts/run_loop.sh" || warn "无法自动停止 run_loop.sh"
    log "已尝试停止 run_loop.sh"
  fi

  # 停止 dashboard/server.py
  if pgrep -f "python.*dashboard/server.py" > /dev/null; then
...
unregister_agents() {
  ...
  log "已备份当前配置: $OC_CFG.bak.pre-uninstall-*"
...
AGENTS_TO_REMOVE = {"taizi", "zhongshu", "menxia", "shangshu", "hubu", "libu", "bingbu", "xingbu", "gongbu", "libu_hr", "zaochao"}
...
remove_data() {
  ...
  read -p "是否需要删除项目内的 data 目录及已生成的数据？(y/N) " -n 1 -r
  echo
  if [[ $REPLY =~ ^[Yy]$ ]]; then
...
sailing@sailing-virtual-machine:~/edict$
